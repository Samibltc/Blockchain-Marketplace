  ...
  describe('Purchasing', () => {
    describe('Success', () => {
      beforeEach(async () => {
        await contract
          .connect(owner)
          .createAppartment(name, description, location, images.join(','), listings, toWei(price))

        const amount = price * dates1.length + (price * dates1.length * securityFee) / 100
        await contract.connect(tenant1).buyProduct(id, dates1, {
          value: toWei(amount),
        })
      })

      it('Should confirm product purchase', async () => {
        result = await contract.getPurchases(id)
      ...
      it('Should confirm qualified reviewers', async () => {
        ...
        await contract.connect(tenant1).approveProduct(id, 1)

        result = await contract.getQualifiedReviewers(id)
        expect(result).to.have.lengthOf(1)
      })

      it('Should confirm product approval', async () => {
        result = await contract.getPurchase(id, purchaseId)
        expect(result.checked).to.be.equal(false)

        result = await contract.connect(tenant1).tenantBooked(id)
        expect(result).to.be.equal(false)

        await contract.connect(tenant1).approveProduct(id, purchaseId)
      ...
      it('Should confirm product refund', async () => {
        result = await contract.getPurchase(id, purchaseId)
        expect(result.cancelled).to.be.equal(false)

        await contract.connect(tenant1).refundPurchase(id, purchaseId)
    ...
    describe('Failure', () => {
      ...
      it('Should prevent purchase with wrong id', async () => {
        const amount = price * dates1.length + (price * dates1.length * securityFee) / 100
        await expect(
          contract.connect(tenant1).buyProduct(666, dates1, {
            value: toWei(amount),
          })
        ).to.be.revertedWith('Product not found!')
      })

      it('Should prevent purchase with wrong pricing', async () => {
        await expect(
          contract.connect(tenant1).buyProduct(id, dates1, {
