...
const generateFakeProduct = (count) => {
  const products = []
  for (let i = 0; i < count; i++) {
    ...
    const listings = faker.number.int({ min: 2, max: 5 })
...
async function createProducts(contract, product) {
...
async function buyProducts(contract, aid, dates) {
  const tx = await contract.buyProduct(aid, dates, { value: toWei(maxPrice * dates.length) })
  await tx.wait()
}
...
async function main() {
  ...
  try {
    ...
    // Process #1
    await Promise.all(
      generateFakeProduct(dataCount).map(async (product) => {
        await createProducts(dappBnbContract, product)
      })
    )
    ...
    await Promise.all(
      Array(dataCount)
        .fill()
        .map(async (_, i) => {
          await buyProducts(dappBnbContract, i + 1, dates1)
        })
    )
